...
## Goal
Create `supervisor/src/font8x16.rs` containing the full 8×16 bitmap font for printable ASCII (0x20–0x7E, 95 glyphs) as a `pub const` byte array. Each character is 16 bytes — one byte per row, MSB = leftmost pixel. Index formula: `FONT[(ch as usize - 0x20) * 16 .. ]`.

## Files to create / modify

```
supervisor/src/font8x16.rs   (new)
supervisor/src/display.rs    (add: mod font8x16 use)
supervisor/src/main.rs       (add: to module list if needed)
```

## Implementation

### `supervisor/src/font8x16.rs`
...
- Font data is the classic VGA 8×16 CP437 font, printable ASCII subset
- `GLYPH_W = 8`, `GLYPH_H = 16` are exported for use in `Framebuffer::draw_text`
- Characters outside `FIRST_CHAR..=LAST_CHAR` render as a blank glyph (all zeros)

## Verification

```sh
# Check font module compiles
docker run --rm --platform linux/amd64 -v "$(pwd)":/work -w /work \
  vyomaos-builder:latest cargo build \
  --manifest-path supervisor/Cargo.toml \
  --target x86_64-unknown-linux-musl --release 2>&1 | grep -E "error|Finished"
```
