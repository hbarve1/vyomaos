...
const DEFAULT_SW: u32 = 1440;  // fallback screen width (used if VYOMA_SYSTEM:screen: never arrives)
...
fn main() {
    ...
    // Runtime screen width — updated by VYOMA_SYSTEM:screen: notification.
    let mut sw: u32 = DEFAULT_SW;
    ...
    loop {
        ...
        if last_fetch.elapsed() >= Duration::from_secs(2) {
            ...
            loop {
                match stdin_lines.next() {
                    Some(Ok(line)) if line.starts_with("VYOMA_SYSTEM:screen:") => {
                        let dims = line.trim_start_matches("VYOMA_SYSTEM:screen:");
                        if let Some((ws, _hs)) = dims.split_once(',') {
                            if let Ok(w) = ws.parse::<u32>() { sw = w; }
                        }
                    }
        ...
        draw(&cached_apps, boot_count, refresh, cursor_pos, sw);
        refresh += 1;
...
fn draw(apps: &[AppInfo], boot_count: u64, refresh: u64, cursor_pos: Option<(u32, u32)>, sw: u32) {
    let running = apps.iter().filter(|a| a.running).count();
    let total   = apps.len();

    // ── Background (dashboard area only) ─────────────────────────────────────
    fill(0, 0, sw, DASH_H, C_BG);

    // ── Header bar ────────────────────────────────────────────────────────────
    fill(0, 0, sw, 64, C_HEADER);               // 52→64 to fit 16×32 large font
    fill(0, 64, sw, 3, C_ACCENT);              // accent line moves down
    fill(16, 14, 32, 32, C_GREEN);             // logo block
    text_large(58, 16, C_WHITE, "VyomaOS");    // 16×32 glyph, y=16 centres in 64px bar
    text(sw - 210, 24, C_DIM, &format!("boot #{}", boot_count));
    ...
    const CARD_H: u32 = 80;
    const GRID_Y: u32 = 94;
    let card_w: u32 = (sw - GAP * (COLS + 1)) / COLS;
    ...
    for (i, app) in apps.iter().enumerate() {
        ...
        let hovered = cursor_pos
            .map(|(mx, my)| find_card_under_cursor(mx, my, apps.len(), sw) == Some(i))
            .unwrap_or(false);
...
/// `sw` is the runtime screen width used for layout calculations.
fn find_card_under_cursor(mx: u32, my: u32, count: usize, sw: u32) -> Option<usize> {
    const COLS: u32   = 4;
    const GAP: u32    = 10;
    const CARD_H: u32 = 80;
    const GRID_Y: u32 = 94;
...
mod tests {
    ...
    fn no_cards_no_highlight() {
        assert_eq!(find_card_under_cursor(100, 100, 0, DEFAULT_SW), None);
    }
    ...
    fn cursor_on_first_card() {
        ...
        assert_eq!(find_card_under_cursor(15, 100, 1, DEFAULT_SW), Some(0));
        assert_eq!(find_card_under_cursor(356, 173, 1, DEFAULT_SW), Some(0));
    }

    #[test]
    fn cursor_in_gap_no_highlight() {
        // card 0 ends at x=357; card 1 starts at x=367; gap is [357, 367)
        assert_eq!(find_card_under_cursor(360, 100, 5, DEFAULT_SW), None);
    }

    #[test]
    fn cursor_above_grid() {
        assert_eq!(find_card_under_cursor(15, 90, 5, DEFAULT_SW), None);
    }

    #[test]
    fn cursor_on_second_card() {
        // card 1: col=1 → cx = 10 + 1*(347+10) = 367, cy=94
        assert_eq!(find_card_under_cursor(370, 100, 5, DEFAULT_SW), Some(1));
    }

    #[test]
    fn cursor_on_fifth_card_second_row() {
        // card 4: col=0, row=1 → cx=10, cy=94+(80+10)=184
        assert_eq!(find_card_under_cursor(15, 190, 5, DEFAULT_SW), Some(4));
    }
}
