    ...
    #[cfg(target_os = "linux")]
    {
        ...
        thread::Builder::new()
            ...
            .spawn(move || {
                ...
                // Helper: forward a message to the currently focused app.
                let forward = |msg: String| {
                    ...
                    match tty.read(&mut buf) {
                        ...
                        Ok(_) => {
                            if buf[0] == 0x1B {
                                ...
                                if b1[0] == 0x5B {
                                    // CSI sequence (\x1b[…): read the final parameter byte.
                                    let mut b2 = [0u8; 1];
                                    if tty.read(&mut b2).unwrap_or(0) == 0 {
                                        continue; // incomplete, discard
                                    }
                                    match b2[0] {
                                        0x41 => forward("\x1b[A".to_string()), // ↑ up arrow
                                        0x42 => forward("\x1b[B".to_string()), // ↓ down arrow
                                        b2b  => {
                                            // Check for window-management CSI sequences.
                                            if let InputAction::AltShiftTab =
                                                classify_input_sequence(&[0x1B, 0x5B, b2b])
                                            {
                                    ...
                                    match classify_input_sequence(&[0x1B, b1[0]]) {
                                        ...
                                        InputAction::AltW => {
                                            ...
                                            if let Some(ref name) = focused_name {
                                                ...
                                                *focused_input.lock().unwrap() = rest.into_iter().next();
                                            }
                                        }
                                        InputAction::AltF => {
                                ...
                                // Non-ESC byte: classify and forward to focused app.
...
fn route_or_print(
    ...
    if has_display {
        if let Some(cmd) = line.strip_prefix("VYOMA_DRAW:") {
            // Compute uptime from registry for the status bar; default to 0 on miss.
            let uptime_secs: u64 = {
                let reg = app_registry.lock().unwrap();
                reg.get(sender)
            ...
            handle_draw_command(cmd, sender, win_region, focused, uptime_secs);
            return;
        }
    }
    let _ = has_display;
...
#[cfg(target_os = "linux")]
fn handle_draw_command(cmd: &str, sender: &str, win: Option<(u32, u32, u32, u32)>, focused: &FocusedApp, uptime_secs: u64) {
    let Some(fb_lock) = display::get() else { return };
